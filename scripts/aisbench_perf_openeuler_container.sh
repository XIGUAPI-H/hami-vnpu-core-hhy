#!/usr/bin/env bash
# ais_bench performance inside openEuler container (sidecar) against running vLLM.
set -euo pipefail
...
VLLM_CONTAINER="${VLLM_CONTAINER:-vnpu-openeuler-so-oe-vllm}"
...
OUT="${BENCH}/outputs/openeuler_oe_vllm_${OUT_TAG}"
LOG="${OE}/logs/aisbench_perf_${OUT_TAG}.log"

mkdir -p "${CFG}/models" "${OE}/logs"
...
models = [
    dict(
        ...
        batch_size=4,
        concurrency=${AISBENCH_CONCURRENCY},
...
echo "=== ais_bench perf (openEuler container) -> 127.0.0.1:${PORT} model=${MODEL} ===" | tee "$LOG"
...
\$PY -m ais_bench.benchmark.cli.main \\
  ...
  -w /benchmark/outputs/openeuler_oe_vllm_${OUT_TAG}
" 2>&1 | tee -a "$LOG"

CSV="$(find "${BENCH}/outputs" -path "*openeuler_oe_vllm_${OUT_TAG}*" -name '*.csv' 2>/dev/null | head -1)"
if [[ -n "$CSV" ]]; then
