#!/usr/bin/env bash
# Build main + burst v5 on openEuler 68 → release-main-burst/
set -euo pipefail
ROOT=/mnt/local/m00953550/hami-vnpu-core-main
OUT=/mnt/local/m00953550/FinalTest/openeuler/release-main-burst
...
if [[ -f "$ROOT/scripts/compare_perf_main_burst.sh" ]]; then
  install -m 755 "$ROOT/scripts/compare_perf_main_burst.sh" "$OE/compare_perf_main_burst.sh"
fi
