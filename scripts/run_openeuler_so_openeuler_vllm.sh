#!/usr/bin/env bash
# openEuler container + openEuler-built libvnpu/limiter + vLLM (vllm-workspace + offline py deps).
set -euo pipefail
...
OE="${FT}/openeuler"
SO="${OE}/release"
NAME="vnpu-openeuler-so-oe-vllm"
...
SO=/opt/ft/openeuler/release
export LD_PRELOAD=${SO}/libvnpu.so
export NPU_GLOBAL_SHM_PATH=/hami-shared-region/global_registry_openeuler_so_oe_vllm
export NPU_LOCAL_SHM_NAME=vnpu_openeuler_so_oe_vllm
...
echo "=== openEuler container + openEuler so ==="
sha256sum ${SO}/libvnpu.so ${SO}/limiter | cut -c1-16
ldd ${SO}/limiter | head -2
rm -f /dev/shm/${NPU_LOCAL_SHM_NAME} 2>/dev/null || true
${SO}/limiter > /opt/ft/logs/limiter-openeuler-oe.log 2>&1 & sleep 5
pgrep -f "${SO}/limiter" || { cat /opt/ft/logs/limiter-openeuler-oe.log; exit 1; }
